...
<template>
    ...
    <div class="px-2 sm:px-3">
      <div class="flex items-center py-2 sm:py-[12px]">
        <div class="flex flex-auto items-center">
          <BaseButtonRound v-if="!domain" class="sm:hidden border-none" @click="showSidebar = !showSidebar">
            <i-ho-menu class="text-skin-link" />
          </BaseButtonRound>
          <router-link :to="{ name: 'home' }" class="items-center font-extrabold text-skin-primary ml-2 text-lg">
            World Association
          </router-link>
          <span v-if="env === 'demo'" class="ml-1 hidden uppercase text-[12px] sm:block mb-1" v-text="'testnet'" />
        </div>
  ...
  <div v-if="pendingTransactions.length > 0"
    class="flex items-center justify-center gap-x-2 bg-skin-border py-2 text-center text-skin-link" :class="{
  ...
  <Teleport to="#modal">
    <ModalPendingTransactions :open="showPendingTransactionsModal" @close="showPendingTransactionsModal = false" />
  </Teleport>
</template>
